...
if ($_SERVER['REQUEST_METHOD'] === "POST") { // formulaire posté
  ...
  if ($last_id != 0) {
    ...
    // on vérifie qu'il y a au moins un buteur pour l'équipe 1
    if (isset($_POST['buteurs1'])) {
    ...
    // on vérifie qu'il y a au moins un buteur pour l'équipe 2
    if (isset($_POST['buteurs2'])) {
      $buteurs2 = $_POST['buteurs2'];
      $minutes2 = $_POST['minutes2'];

      for($i = 0; $i < sizeof($buteurs2); $i++) {
        $donnees = array(
          'rencontre' => $rencontre->getId(),
          'joueur' => $buteurs2[$i],
          'minute' => $minutes2[$i]
